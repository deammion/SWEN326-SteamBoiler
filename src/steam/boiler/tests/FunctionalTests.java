...
public class FunctionalTests {
	 ...
	 *
	 * @param time          The time (in s) to operate the boiler before checking
	 *                      the levels.
	 * @param numberOfPumps The number of pumps to use in this configuration.
	 * @param config        The boiler characteristics to use for this run.
	 */
	private void test_normal_operation(int time, int numberOfPumps) {
		...
		// In an ideal setting, we expect the system to keep the level within the normal
		// range at
	...
	public void test_degraded_operation_03() {
		SteamBoilerCharacteristics config = SteamBoilerCharacteristics.DEFAULT;
		// Configure only two pumps with combined capacity of 8L, which is not enough
		// when steam is at
		// full exhaust.
		...
		model.setMode(PhysicalUnits.Mode.WAITING);
		// Clock system for a given amount of time. This is tricky, as we don't want to
		// allow an
		// emergency stop to happen. Difference between min normal and min limit is
		// 100L. Using both
		// pumps fully puts in 8L/s, whilst 10L/s is taken out in exhaust. Therefore,
		// 2L/s drop in water
	...
	public void test_degraded_operation_05() {
		SteamBoilerCharacteristics config = SteamBoilerCharacteristics.DEFAULT;
		// Configure only three pumps with combined capacity of 12L, which is just
		// enough when steam is
		...
		// Now, break the pump controller by fixing it closed.
		model.setPumpController(0, new PumpControllerModels.StuckOff(0, model));
		// At this point, we can't tell how long it will be before the system realises
		// the pump
		// controller is broken. For example, if the pump is currently off then there
		// will be no
		// discrepancy between the controller and the pump. Eventually, the pump will
		// come back on and
	...
	private void test_degraded_operation(MySteamBoilerController controller, SteamBoilerCharacteristics config,
		...
		// wrong during this time.
		clockForWithout(time, controller, model, atleast(MODE_emergencystop));
		// Even in this degraded setting, we expect the system to keep the level within
		// the normal range
		// at all times. Therefore, check water level is indeed within normal range.
		if (model.getBoiler().getWaterLevel() > config.getMaximalLimitLevel()) {
			fail("Water level above limit maximum (after " + time + "s with " + config.getNumberOfPumps() + " pumps)");
		}
		if (model.getBoiler().getWaterLevel() < config.getMinimalLimitLevel()) {
			fail("Water level below limit minimum (after " + time + "s with " + config.getNumberOfPumps() + " pumps)");
		}
	}
	...
	// or degraded more. Likewise, running the system in rescue more for some amount
	// of time to check
